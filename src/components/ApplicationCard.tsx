import { ApplicationCardProps } from '@/types/applicationProps'
import Link from 'next/link'
import React from 'react'

const ApplicationCard: React.FC<ApplicationCardProps> = ({ offer, status }) => {
    return (
        <article key={offer.id} className="flex max-w-xl flex-col items-start justify-between">
            ...
            <p className="rounded-xl bg-[#00b140] px-5 py-2 text-base font-medium text-white transition duration-200 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:text-white dark:hover:bg-brand-300 dark:active:bg-brand-200">
                {status}
            </p>
        </article>
    )
